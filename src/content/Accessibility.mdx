...
## resources studied
	- [RSC_23m](https://youtu.be/VyWRmepESoQ?si=ydEtvTwmZ9D4MlMH)
	- [WDS_13m](https://youtu.be/1A6SrPwmGpg?si=0BycOeGqJp9hLCsJ)
	- [JSer_Tabs_43m](https://youtu.be/QKlJNuckVcU?si=9HjF374H_CYMO2la)
	- [WDS_37m](https://youtu.be/2oiBKSjOOFE?si=FS9BwpnOq3eiSzmX)
...
## tools
	- chrome lighthouse (not fully useful)
	- jaws (paid), or use system screen readers with eyes closed
	- `eslint-plugin-jsx-a11y` (similar for html etc)
## use correct HTML semantics like `nav`, `aside`
	- a page should have only 1 `h1` tag
	- main, aside, article, button (not everything should be a div)
...
## inert for hidden components like side nav content, modal content
	- essentially does tabIndex = -1 for all the children not on screen
	- `aria-hidden=true`, `role=dialog`, `aria-modal=true` handle inertness automatically
